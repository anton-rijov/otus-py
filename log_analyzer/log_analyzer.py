...
def get_url(line):
    ...
    if search_result:
        return search_result.group(2)[0:128]
    else:
        return f'wrong url on line: {line}'


def get_duration(line):
    return float(re.split(r" ", line)[-1])


def gen_parser(lines):
    for line in lines:
        yield get_url(line), get_duration(line)
...
def squeeze_out(lst):
    ...
    lst = lst[:config["REPORT_SIZE"]]
    return lst


def make_report():
    filenames = gen_find(config["LOG_FILE_MASK"], config["LOG_DIR"])
    ...
    report_file_name = os.path.join(config["REPORT_DIR"], "report-" + date[0:4] + '.' + date[4:6] + '.' + date[6:8] + '.html')
    if os.path.isfile(report_file_name):
        logger.info(f"report {report_file_name} already exists.")
        return
    result_list = collect_statistics(recent)
    if not result_list:
        logger.error("empty statistic data")
        return
    json_array = json.dumps(squeeze_out(result_list))
    try:
...
def main():
    logger.info(os.path.basename(__file__) + " started.")
    make_report()
